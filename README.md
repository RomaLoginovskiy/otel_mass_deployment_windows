...
| `poc/` | VirtualBox harnesses to validate the package on a throwaway Windows Server 2025 VM before fleet rollout — including `Run-FullMatrixVmLoop.ps1`, the **asserting full matrix** that no container can run: both install modes, every IIS runtime/CLR combination, PM2, Node-as-a-service, a .NET service, collector *and* supervisor health, reboot survival, and telemetry gated by **querying** Coralogix. See [`docs/vm-e2e-matrix.md`](docs/vm-e2e-matrix.md). |
