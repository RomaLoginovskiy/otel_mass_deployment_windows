...
function Get-CxQueryKey {
    <#
      ...
      Dynatrace dt0c01 token, which is a different vendor's key that happens to share the file.
    #>
    [CmdletBinding()]
    param(
    ...
    if (-not $candidates.Count) {
        throw "no Coralogix (cxup_) key found in $Path. Lines look like 'label - cxup_...' or a bare token; a Dynatrace dt0c01 key is not usable here."
    }
