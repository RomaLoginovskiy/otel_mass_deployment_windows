# How this deployment compares to Dynatrace OneAgent

Written from a reverse-engineering study of OneAgent 1.341.56 for Windows (the full corpus lives in
`C:\Users\roman\Documents\projects\dynatrace\docs`, notably `11-INVOCATION-GRAPH.md` and
...
## What we now do that OneAgent also does

| Behaviour | OneAgent's version | Ours |
| --- | --- | --- |
...
| Skip tool processes | `[blocklist]` in `ruxitagentproc.conf`, app + exe filters | `deploy/cx-node-blocklist.txt`, anchored patterns, plus built-in defaults |
...
## What OneAgent does that we deliberately do not
...
- **`AppInit_DLLs` / `AppCertDlls` / any hooking DLL** — `AppInit_DLLs` never loads in `node.exe` (it has
  ...
  `oneagentproc.dll`, which is a different product.
...
instrumentation (OneAgent's `oneagentiis.dll` injects a JS agent into HTML responses; we do not),
method-level sensors, live debugging, and cluster-pushed per-sensor configuration.

## Signal coverage: the honest delta

OneAgent ships ~130 `Introspection.*` sensor assemblies. The naive comparison ("they have WCF, MSMQ,
...
On the Node side OneAgent covers log enrichment for pino/winston/bunyan/log4js, `undici`/`fetch`, HTTP/2,
...
Only **one** `ICorProfilerCallback` can attach to a process. On a host where OneAgent has .NET deep
...
Worth knowing: OneAgent is *built* to tolerate an OTel SDK in-process. It ships
...
OneAgent ships `oneagentctl.exe` with **56 verbs**, every one a symmetric get *and* set — monitoring
...
One detail worth copying that we have not: OneAgent's watchdog distinguishes a **suspended VM** from a
...
- [reference/exit-codes.md](reference/exit-codes.md) — every finding code, including the ones added from
  this comparison
- `dynatrace/docs/11-INVOCATION-GRAPH.md`, `12-GUARDRAILS-AND-CONFIG.md` — the evidence base
